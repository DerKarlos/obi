...
use crate::obi_api_out::{GpuPosition, OsmMesh};
...
impl OsmMesh {
    pub fn push_onion(
        ...
        for point in shape_curve {
            ...
            for column_point in roof_polygon.iter() {
                // push colors
                self.vertices_colors.push(color);
                self.vertices_colors.push(color);
                // push vertices
                let pos_x = (column_point.x - pike.x) * curve_radius + pike.x;
                ...
                // push indices
                let index = self.vertices_positions.len() as i32;
                self.vertices_positions.push(last_pos); // right vertice different than left to get corneres
                self.vertices_positions.push(this_pos); // left - up=down to not get corners
                last_pos = this_pos;
                ...
                if curve_radius > 0. {
                    ...
                    self.indices_to_vertices.push(index as u32); // 0     2
                    self.indices_to_vertices.push((index + 1) as u32); // 1     | \
                    self.indices_to_vertices
                        .push((index + to_next_column) as u32); // 2     0---1
                                                                // Secound treeangle
                    self.indices_to_vertices.push((index + 1) as u32); // 0     2---1
                    self.indices_to_vertices
    ...
    pub fn push_flat(
        ...
        let roof_index_offset = self.vertices_positions.len();
        for position in positions {
            self.vertices_positions.push(position);
            self.vertices_colors.push(color);
        }
    ...
    pub fn push_pyramid(
        ...
        self.vertices_positions.push(pike);
        self.vertices_colors.push(color);
    ...
    pub fn push_square(
        ...
        let index = self.vertices_positions.len();
        // Push the for colors
        self.vertices_colors.push(color);
        self.vertices_colors.push(color);
        self.vertices_colors.push(color);
        ...
        self.vertices_positions.push(down_left); //  +0     2---3
        self.vertices_positions.push(down_right); // +1     |   |
        self.vertices_positions.push(up_left); //    +2     0---1
        self.vertices_positions.push(up_right); //   +3
                                                // First treeangle
        self.indices_to_vertices.push((index + O) as u32); //     2
        self.indices_to_vertices.push((index + 1) as u32); //     | \
        self.indices_to_vertices.push((index + 2) as u32); //     0---1
                                                           // Secound treeangle
        self.indices_to_vertices.push((index + 1) as u32); //     2---3
        self.indices_to_vertices.push((index + 3) as u32); //       \ |
        self.indices_to_vertices.push((index + 2) as u32); //         1
    }
}
