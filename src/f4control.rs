...
fn player_keys(
    ...
    if let Ok(_window) = primary_window.single() {
        const SPEED_DEGREE_PER_M: f32 = 1.0 / 200000.0;
        let speed = control_values.speed;
        let view = &mut control_values.view;
        let elevation_fakt = 1. + time.delta_secs_f64() / 1.0;
        let groundmove_fact_lat = speed * time.delta_secs_f64() * SPEED_DEGREE_PER_M;
        ...
        let rotation_fact = time.delta_secs_f64() * 20.0; // delta time * degrees per second = delta degrees
