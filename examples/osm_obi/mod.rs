...
use osm_tb::{scan_objects, InputJson};
...
impl rend3_framework::App for ObiExample {
    ...
    fn setup(&mut self, context: rend3_framework::SetupContext<'_>) {
        ...
        let building_parts = api.scan_osm_vec(&bytes, &bounding_box.center_as_geographic_coordinates(), 0);
        ...
        let mut rend_vertex_positions: Vec<glam::Vec3> = Vec::new();
        for vertex_position in vertex_positions {
            let rend_vertex_position =
                glam::Vec3 { x: vertex_position[0], y: vertex_position[1], z: vertex_position[2] };
            rend_vertex_positions.push(rend_vertex_position)
        }
        let index_data = &meshes[0].indices_to_vertices.clone();
        let mesh = rend3::types::MeshBuilder::new(rend_vertex_positions, rend3::types::Handedness::Left)
        ...
        let x = 30.;
        let view_location = glam::Vec3::new(3.0 * x, 3.0 * x, -5.0 * x);
        let view = glam::Mat4::from_euler(glam::EulerRot::XYZ, -0.35, 0.5, 0.0);
...
pub fn main() {
    ...
    rend3_framework::start(app, winit::window::WindowBuilder::new().with_title("obi-example").with_maximized(true));
}
