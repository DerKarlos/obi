...
fn main() {
    ...
    web_sys::console::log_1(&"Fetching JSON data...".into());

    // The input API
    let api = OsmApi::new();

    // Get the center of the GPU scene
    let url = api.way_url(121486088);
    dbg!(&url); // ??? Were is it visible?
    println!("url: {url}"); // ??? Were is it visible?
    #[cfg(target_arch = "wasm32")]
    {
        web_sys::console::log_1(&"... using URL: ".into());
        web_sys::console::log_1(&url.into());
    }

    //let response = reqwest::get(url).await;
    //println!("response: {:?}", response);
    //let json_bbox_data: JsonData = response.json();
...
fn setup(
    ...
    commands.spawn((
        ...
        Transform::from_xyz(1.5, 0.51, 0.5),
    ));
...
% cargo build --release --target wasm32-unknown-unknown    !!! AFTER EDIT 2 LINES: !!!
% wasm-bindgen --out-dir ./out/ --target web ./target/wasm32-unknown-unknown/release(/examples)/<name>.wasm
% npx serve .                                              !!! AT THE OUT DIR      !!!
